...
async function editFluffy() {
  try {
    ...
    await writeOutput('weightOutput', parsedContent);

    parsedContent.name = "Fluffyy";
    await writeOutput('nameCorrectionOutput', parsedContent);
    ...
    await writeOutput('moreActivitiesOutput', parsedContent);

    let [updatedBar] = parsedContent.catFriends;
    updatedBar.furcolor = "Brown";
    await writeOutput('barColorOutput', parsedContent);
